...
# %% [markdown]
# # 06 · Switchback experiments — *temporal* interference & variance reduction
#
# **NB 06** of the curriculum. NB 05 handled *spatial* interference (a marketplace where treating one user
# cannibalizes others) and fixed it by randomizing whole markets — but that gives a sample size of ~2
# markets. A **switchback** holds the market fixed and **randomizes the treatment over *time*** (on/off
# across periods), so the same market is its own control. You trade cross-sectional interference for
# **temporal** problems: **carryover**, autocorrelation, and a finite lumpy set of cells.
#
# A **practical** companion to the primer (`paper-library/papers/switchback-and-variance-reduction.md`).
# We re-implement the DoorDash switchback simulator (Pankratev 2026) and *learn by doing* how the
# variance-reduction family **Raw → CUPED → CUPAC → DML-DR** buys power, plus the two non-obvious lessons
# (target *macro* shocks, and the efficiency-vs-robustness trap). Self-contained — we author the DGP, so we
# have **ground truth** (the true τ and the latent shocks) to validate against, like the rest of Lyra.
#
...
# Type-S) · (7) recommendations · (8) **promote** to `lyra/dgp` + the platform.
...
# %% [markdown]
# ## 8. Promote → `lyra/dgp` + the platform
#
# The simulator becomes **`SwitchbackDGP`** (`lyra/dgp`, `ground_truth()` = τ) and the realistic CUPED
# analysis becomes **`SwitchbackCUPED`** (`lyra/estimators_vr`: CUPED on `x_hist` + cluster-robust OLS). The
# harness certifies both against the known τ; then a **switchback experiment is added to the chassis**, with
# the CUPED variance reduction visible.
...
print("promoted SwitchbackDGP (truth τ = 20):")
print(f"  raw   cluster-robust : coverage {raw['coverage']:.2f}   CI width {raw['ci_width']:.1f}")
print(f"  CUPED switchback     : coverage {cup['coverage']:.2f}   CI width {cup['ci_width']:.1f}   "
      f"(variance reduction {1 - (cup['ci_width'] / raw['ci_width']) ** 2:.0%})")

# %% [markdown]
# ✅ **Promoted** (covered by `tests/test_switchback.py`). Both designs are certified against the known τ;
# CUPED keeps the same coverage while cutting the interval ~40%. The chassis now carries a **switchback**
# experiment — temporal interference, handled and validated end-to-end. **Next (`ROADMAP.md`): NB 07 —
# sequential & diagnostics** (peeking-safe inference; recycle `_old/03` + `inference/anytime_valid`).
